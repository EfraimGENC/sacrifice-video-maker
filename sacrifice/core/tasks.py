...
def make_animal_video(animal_id: int, force: bool = False):
    ...
    new_video_path, processing_status = animal_service.process_animal(animal)
    logger.info(f'make_animal_video - processing is finished: {animal}')

    animal_service.finnish_animal_processing(animal_id, new_video_path, processing_status)
